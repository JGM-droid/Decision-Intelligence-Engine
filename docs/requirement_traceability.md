...
| Report at least 3 task-appropriate metrics per model (for example accuracy, precision, recall, F1, top-k accuracy). | COMPLETE AFTER THIS AUDIT | [docs/architecture_comparison.md](docs/architecture_comparison.md), [docs/mlflow_tracking.md](docs/mlflow_tracking.md#run-evidence) | Macro precision, macro recall, and macro F1 are surfaced as post-hoc test-set metrics from the saved confusion matrices; per-run values are in the tracked MLflow evidence table. |
...
| Log at least 5 meaningfully different experiment runs. | COMPLETE | 9 FINISHED runs across 7 configurations with run IDs and macro metrics in [docs/mlflow_tracking.md](docs/mlflow_tracking.md#run-evidence); [docs/mobilenetv2_experiments.md](docs/mobilenetv2_experiments.md), [docs/architecture_comparison.md](docs/architecture_comparison.md) | None. |
